...
@dish_router.get('', response_model=list[Dish_Pydantic], status_code=status.HTTP_200_OK, summary='get_dishes_list', name='get_all_dishes')
...
@dish_router.get('/{target_dish_id}', response_model=Dish_Pydantic,
                 status_code=status.HTTP_200_OK, summary='Get dish by id', name='get_dish_by_id',
                 responses={status.HTTP_404_NOT_FOUND: {'description': 'Dish with given id was not found', 'model': ResponseDishNotFoundError}})
...
@dish_router.post('', response_model=Dish_Pydantic, summary='Create new dish', name='create_dish',
...
@dish_router.patch('/{target_dish_id}', response_model=Dish_Pydantic, status_code=status.HTTP_200_OK,
                   summary='update_dish_by_id', name='change_dish',
                   responses={status.HTTP_404_NOT_FOUND: {'description': 'Dish with given id was not found', 'model': ResponseDishNotFoundError},
...
@dish_router.delete('/{target_dish_id}', response_model=str, status_code=status.HTTP_200_OK, summary='delete_dish_by_id', name='delete_dish')
